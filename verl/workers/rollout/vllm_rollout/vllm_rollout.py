...
class vLLMRollout(BaseRollout):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.pattern = re.compile(r".*?return[^\n]*", re.DOTALL)
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            #        self.inference_engine.get_tokenizer().tokenizer.decode(res)
            #    )
            #    if m:
            #        return pad_sequence_to_length(
            #            torch.cat(
            #                (
            #                    torch.tensor(
            #                        self.inference_engine.get_tokenizer().tokenizer.encode(
            #                            m.group(), add_special_tokens=False
            #                        ),
            #                        device=idx.device,
            #                    ),
            #                    torch.tensor(eos_token_id, device=idx.device).unsqueeze(
            #                        0
            #                    ),
            #                )
            #            ),
            #            self.config.response_length + 10,
            #            self.pad_token_id,
            #        )

            #    else:
            #        return pad_sequence_to_length(
            #            res, self.config.response_length + 10, self.pad_token_id
            #        )

            # print(
            #    [
            #        self.inference_engine.get_tokenizer().tokenizer.decode(response[i])
            #        for i in range(3)
            #    ],
            #    flush=True,
            # )
            # response = torch.vstack(list(map(process_res, response)))
            # print(
            #  self.inference_engine.get_tokenizer().tokenizer.decode(response[0]),
            #  flush=True,
            # )
            # print(
            #    [
